...
use crate::tunnel::traffic::{self, TrafficCounters, TrafficHistory};
use crate::types::{AppConfig, AuthMethod, TrafficSample, TunnelConfig, TunnelInfo, TunnelStatus};
...
impl TunnelManagerActor {
    ...
    async fn handle_connect(&mut self, id: &str) -> Result<(), TunnelError> {
        ...
        // Spawn traffic sampler — always spawn since event_handler is always present
...
mod tests {
    ...
    fn noop_handler() -> Arc<dyn TunnelEventHandler> {
    ...
    async fn list_tunnels_returns_all() {
        let handle = spawn_manager(test_config(), noop_handler());
    ...
    async fn connect_to_unreachable_host_returns_error() {
        ...
        let handle = spawn_manager(config, noop_handler());
