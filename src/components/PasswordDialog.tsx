...
export function PasswordDialog({ tunnelName, isRetry, onSubmit, onCancel }: PasswordDialogProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50">
      <form onSubmit={handleSubmit} className="bg-white dark:bg-[#1a1a1a] rounded-xl p-4 mx-3 w-full border border-[rgba(0,0,0,0.06)] dark:border-[rgba(255,255,255,0.08)]">
        ...
        {isRetry && (
          <p className="text-xs text-[#dc2626] dark:text-[#f87171] mb-2">Incorrect password. Please try again.</p>
        )}
